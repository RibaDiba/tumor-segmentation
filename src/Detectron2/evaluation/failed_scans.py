...
sys.path.insert(
    0, os.path.join(os.path.dirname(__file__), "../../../util/preprocessing")
)
...
class FailedScans(Dataset):
    ...
    def create_graph(self):
        ...
        v_original = Visualizer(im[:, :, ::-1], metadata=self.test_metadata, scale=0.5)
        ...
        model_name = (
            os.path.basename(os.path.dirname(self.model_path))
            if self.model_path
            else "Model"
        )
    ...
    def init_metadata(self):
        ...
        self.cfg.DATALOADER.FILTER_EMPTY_ANNOTATIONS = (
    ...
    def init_model(self):
        ...
        self.cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST = (
            0.5  # set a custom testing threshold
        )
...
def main():
    ...
    fs = FailedScans(
        ...
        output_path="",
        image_paths=[
            "/projects/PUCHALLA/LLP2024/tumor-segmentation/data/huggingface-repo/examples_used_failed/M7-1_texture_1.jpg"
        ],
    )
