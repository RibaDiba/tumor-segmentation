...
class LossVisualizationHook(HookBase):
    ...
    def after_step(self):
        ...
        mask_loss = current_scalars["mask_loss"][0]
    ...
    def _save_plots(self):
        ...
        fig.set_title(f"Losses for {self.model_name}")
        fig.suptitle(f"Trained with {self.trainer.max_iter}")

        axs[0].plot(self.loss_dict_total.keys(), self.loss_dict_total.items())
        axs[0].set_title("Total Loss Plot")
        axs[1].plot(self.loss_dict_mask.keys(), self.loss_dict_total.items())
        axs[1].set_title("Mask Loss Plot")

        fig.savefig(os.path.join(self.output_dir, f"{self.model_name}'s plot"))
