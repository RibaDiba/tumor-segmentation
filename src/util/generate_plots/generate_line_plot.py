...
TITLE      = "Base 5000 images (No Depth)"  # used for graph title and output filename
SHOW_DEPTH = False  # set to False to hide the DEPTH model
SHOW_DIFF  = True  # set to True to plot per-image IoU difference vs RGB

BASE = Path(__file__).resolve().parents[3] / "src/Detectron2/slurm_output/AUG_5000"
...
MODELS = {k: v for k, v in _ALL_MODELS.items() if k != "DEPTH" or SHOW_DEPTH}
