...
def infuse_depth_into_blue_channel(image_array, depth_array, folder_path, base_name):
    ...
    for i in tqdm(range(len(image_array)), desc="Infusing Images"):
        ...
        if b.shape == depth_map_normalized.shape:
            ...
            raise ValueError(
                "Dimension mismatch between the blue channel and the depth map."
            )
...
def main():
    ...
    save_contour_grayscale(
        data_array, folder_path="./data/contour_plots/grayscale", base_name="grayscale"
    )
    ...
    save_contour_rgbd(
        ...
        folder_path="./data/contour_plots/rgbd",
        image_array=images,
        base_name="rgbd",
    )
